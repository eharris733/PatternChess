...
export function DemoResult({ demo }: Props) {
  ...
  if (demo.status === 'error') {
    ...
    return (
      <div className="border-2 border-[#1A1A1A] bg-white p-6">
        ...
        <p className="text-[#1A1A1A] mb-4">{demo.error?.message}</p>
        <button
          onClick={demo.reset}
          className="font-mono uppercase tracking-tight text-xs border-2 border-[#1A1A1A] bg-transparent text-[#1A1A1A] px-4 py-2 rounded-none hover:bg-[#1A1A1A] hover:text-[#F4F4F0] transition-colors"
        >
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:justify-between border-2 border-[#1A1A1A] bg-[#1A1A1A] text-[#F4F4F0] p-6">
        <div>
          <div className="font-mono uppercase tracking-tight text-sm sm:text-base">
            Save these blunders.
          </div>
          <div className="font-sans text-[#F4F4F0]/70 text-sm mt-1">
...
function SignUpCta({ children }: { children: React.ReactNode }) {
  return (
    <Link
      to="/login"
      className="self-start font-mono uppercase tracking-tight text-sm border-2 border-[#F4F4F0] bg-gold-dark text-[#F4F4F0] px-6 py-3 rounded-none shadow-[3px_3px_0_#F4F4F0] hover:shadow-[1px_1px_0_#F4F4F0] hover:translate-x-[2px] hover:translate-y-[2px] transition-all whitespace-nowrap"
    >
      {children}
    </Link>
  );
