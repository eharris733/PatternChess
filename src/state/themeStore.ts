...
/**
 * Whether this device has an explicit, persisted theme choice. Decides who wins
 * when the local theme and the signed-in profile's theme disagree: a device that
 * has chosen a theme keeps it (and syncs it up), so a stale/default server value
 * can never clobber the user's choice across sessions.
 */
