...
- **Leaderboards** (`/achievements?tab=leaderboards`): `leaderboard(metric, win, limit_n)` security-definer RPC (migration `20260915140000`) returns `{ rows: [{rank,label,value,isMe}], me }` — label is lichess → chess.com → display name → "Player"; never ids or emails. Metrics: `solved` (lifetime `sum(times_correct)`; week = `training_sessions.blunders_correct`), `mastered` (cycle ≥ 4 and ≥80% recall, same as `getBlunderStats`), `elo` (landing_stats' positive-delta rule per user), `streak` (longest all-time / current this week). `profiles.leaderboard_opt_out` ("Show me on leaderboards", inverted) excludes a user server-side. Client: `src/services/db/leaderboard.ts` + `useLeaderboard` + `LeaderboardPanel`.
- **Social achievements**: `follow-instagram` unlocks on click-through (`profiles.followed_instagram`, `INSTAGRAM_URL` in `achievements.ts`); `share-1/5/25` count `profiles.shares_count`, bumped by the atomic `increment_shares_count()` RPC from the training share handler.
...
As of 2026-09-15 `supabase migration list` also shows the `20260912*` local files missing from remote history and four remote-only entries (`20260912152006`…`152724`); the three 2026-09-15 migrations (`profiles_sounds`, `endgame_scenarios_verification`, `leaderboards_and_social`) are pending. Reconcile with `migration repair` (or apply each via the SQL editor / MCP `apply_migration`) rather than a blind `db push`.
